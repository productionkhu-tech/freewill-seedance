...
import { VideoPlayer, ClipStamp, downloadClip, revealClipFile, mediaSrcFor , originMaybeAlive , posterSrcFor } from './ChatArea';
        ...
        {rows.length === 0 ? (
            ...
            {visible.map(r => (
              <div key={r.id}
                ...
                <div className="aspect-video bg-black relative group">
                  <VideoPlayer src={mediaSrcFor(r)} fallbackSrc={originMaybeAlive(r) ? r.videoUrl : undefined} poster={posterSrcFor(r)} posterOf={r} className="w-full h-full" is4k={r.usedSettings?.resolution === '4k'} />
          ...
          {hasMore && (
            <div ref={sentinelRef} className="py-6 flex items-center justify-center gap-2 text-[12px] text-gray-400">
              아래로 스크롤하면 {rows.length - shown}개 더
            </div>
          )}
