...
> ### ★ 미디어 길이 상한은 API 에게 직접 물어라 (무과금)
> BytePlus 는 검증을 **`resolution` → `ratio` → `duration` → 미디어** 순으로 한다.
> (§5-2 의 무과금 프로브가 `ratio` 를 안전핀으로 쓰는 이유이자, 그 프로브로는 **미디어 길이를
> 잴 수 없는** 이유다 — 30초짜리 비디오도 ratio 에서 먼저 걸려 통과처럼 보인다. 실측 2026-08-07.)
> 길이를 재려면 **파라미터를 전부 정상으로 두고 과길이 미디어를 보낸다.** 초과면 HTTP 400 +
> **task 미생성 = 과금 0** 이고, 메시지가 숫자를 그대로 말해준다:
> `"audio duration (seconds) ... must be less than or equal to 30.2 for model dreamina-seedance-2-5"`
> 개별과 합산이 별도 메시지(`audio duration` / `audio total duration`)로 나온다.
> **주의**: 한도 이내면 task 가 실제로 생성되어 과금된다. 큰 값부터 내려가며 재라.
>
...
| 레퍼런스 **오디오** 개별·합산 | **둘 다 15.2초** | **둘 다 30.2초** |

- **"네이티브 4K" 는 거짓**(1080p·2k·4k 전부 거부). **"50 에셋" 은 사실.**
- 비디오 10개는 사실상 도달 불가 — 합산 30.2초가 먼저 걸린다. 오디오도 같다.
